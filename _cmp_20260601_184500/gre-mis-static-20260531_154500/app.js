...
function trackImpactCounter(counterKey, delta = 1) {
...
function extractSharedSolutionHints(value) {
  ...
  const solutionsSharedMatch = text.match(/solutions shared\s*:\s*([\s\S]*?)(?:solution links shared with seeker|$)/i);
...
class GreMisStore {
  ...
  async saveMailTemplates(providerIntroTemplate, curatorForwardTemplate, solutionSeekerTemplate, needSeekerTemplate, inboundAutoSyncEnabled) {
                ...
                ? `<div class="detail-links">
                    ...
                    <ol class="detail-link-list">
                      ${sharedSolutionEntries
                        .map((entry) => `<li>${esc(entry.offeringName)}${entry.providerName ? ` : ${esc(entry.providerName)}` : ""}${entry.inferred ? ` <span class="helper-text">(GRE-aligned)</span>` : ""}</li>`)
                        .join("")}
                    </ol>
                  </div>`
                : ""
            ...
            <article class="match-card ${match.matchScore >= 100 ? "match-score-high" : "match-score-medium"}">
              ...
              <div class="card-actions match-actions">
                ...
                ${window.APP_CONFIG?.ENABLE_EMAIL_ACTIONS && (hasAdminLikeAccess() || isCuratorUser()) && seekerEmailAction.to
                  ? `<button class="btn btn-secondary" data-action="email-seeker" data-offering-id="${esc(match.offering_id)}">${esc(seekerEmailAction.actionLabel)}</button>`
                  : `<button class="btn btn-secondary" type="button" disabled title="Seeker outreach is available only when a seeker email is stored for this need.">Email to Seeker</button>`}
              </div>
            </article>
          `;
        })
...
function bindStaticEvents() {
  ...
  byId("saveMailTemplatesBtn")?.addEventListener("click", safeAsync(async () => {
    ...
    const result = await store.saveMailTemplates(providerTemplate, curatorTemplate, solutionSeekerTemplate, seekerTemplate, autoSyncEnabled);
    state.mailTemplates = {
  ...
  byId("matchResults")?.addEventListener("click", safeAsync(async (event) => {
    ...
    if (button.dataset.action === "view-match-details") {
      await trackImpactCounter("solutions_discovered");

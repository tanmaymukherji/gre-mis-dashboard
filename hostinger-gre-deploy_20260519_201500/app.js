...
function getGrameeeAccessToken() {
...
class GreMisStore {
  ...
  async bridgeGrameeeSession() {
    const accessToken = getGrameeeAccessToken();
  ...
  async sendProviderIntro(needId, providerEmail, providerName = "", offeringId = "") {
    return this.callAdmin("sendProviderIntro", { needId, providerEmail, providerName, offeringId });
  }
            ...
            <article class="match-card ${match.matchScore >= 100 ? "match-score-high" : "match-score-medium"}">
              ...
              <div class="card-actions match-actions">
                ...
                ${window.APP_CONFIG?.ENABLE_EMAIL_ACTIONS && (hasAdminLikeAccess() || isCuratorUser()) && email
                  ? `<button class="btn btn-primary" data-action="email-provider" data-provider-email="${esc(email)}" data-provider-name="${esc(match.trader?.organisation_name || match.trader?.trader_name || "")}" data-offering-id="${esc(match.offering_id)}">Email This Provider</button>`
                  : `<button class="btn btn-primary" type="button" disabled title="Provider outreach is available only to signed-in curators, moderators, and admins.">Email This Provider</button>`}
              </div>
            </article>
          `;
        })
  ...
  workbench.innerHTML = `
    ${hasAdminLike ? `
      <article class="action-card">
        ...
        <p class="helper-text">${isAdmin ? "Admin can rebalance or assign needs directly." : "Moderators can assign and rebalance curator ownership, but cannot delete or reject records."}</p>
      </article>` : ""}
...
function bindStaticEvents() {
  ...
  byId("matchResults")?.addEventListener("click", safeAsync(async (event) => {
    ...
    if (button.dataset.action === "email-provider") {
      ...
      const result = await store.sendProviderIntro(
